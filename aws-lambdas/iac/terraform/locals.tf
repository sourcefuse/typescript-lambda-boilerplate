locals {
  ...
  lambda_ec_vpc_config = var.custom_ec_vpc != null ? var.custom_ec_vpc : tomap({
    subnet_ids         = [try(module.ec-subnets.private_subnet_ids, "")]
    security_group_ids = [try(module.vpc.vpc_default_security_group_id, "")]
  })
